...
**No. This one bit, and the first answer recorded here was wrong.**

The question anticipated a depot or driver visibility problem. The actual
failure was neither, and it is worth writing down carefully because it looked
like a flake twice before it was understood.

`sudo -E julia bin/install.jl --with-deps webkit` appeared to work: the Linux
WebKit job went green and this file first recorded OQ 7 as answered. It was
green by luck — a warm depot cache meant nothing needed resolving. On a cold
cache the same command failed, and then kept failing:

```
Info: Playwright.jl not in the active environment; activating the checkout
  Activating project at `~/work/playwright-julia/playwright-julia`
    Updating `~/work/playwright-julia/playwright-julia/Project.toml`
    Updating `~/work/playwright-julia/playwright-julia/Manifest.toml`
      [2a0f44e3] ~ Base64 ⇒ v1.11.0
```

Two things went wrong at once:

1. **`sudo` replaces `PATH` from `secure_path`, and `-E` does not stop it.**
   So `julia` under sudo was *not* the Julia `setup-julia` had installed — the
   `v1.11.0` stdlib versions above give it away, on a job pinned to 1.10.
2. **`bin/install.jl` fell back to activating and instantiating the checkout**,
   because Playwright was not in the active environment. Under sudo that
   rewrote `Project.toml` and `Manifest.toml` **as root**, with a manifest
   resolved by the wrong Julia. Every later step then failed on
   `Could not locate the source code for the OpenSSL_jll package. Are you
   trying to use a manifest generated by a different version of Julia?` — a
   message that points nowhere near the cause.

The fix is three small things, all in the workflow rather than the package:

- `$(which julia)`, resolved before sudo, to defeat `secure_path`;
- `--project=.` plus `julia-buildpkg` moved *ahead* of the install steps, so
  the script finds Playwright and never reaches its instantiate fallback —
  nothing should run Pkg as root;
- a `chown` afterwards, because the driver lives in a Scratch.jl directory
  inside the depot and anything root touched there has to go back.

**The lesson generalises past this milestone:** `sudo -E <script that might
resolve a Julia project>` is a trap, and the failure surfaces far away from the
sudo line. It was correctly identified in the spec as "a five-minute question
with a day-shaped answer if it is discovered at Checkpoint C" — and it was
discovered at Checkpoint C.

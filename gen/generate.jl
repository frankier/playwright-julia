...
function subtype_closure(entries)
    ...
    parent = Dict(
        n => (p = get(entries[n], "extends", nothing); p === nothing ? nothing : String(p)) for n in interfaces
    )
...
function map_type(node, entries, unmapped::Set{String})
    ...
    if base in ("object", "array")
        return SpecType(
            base == "array" ? "AbstractVector" : "AbstractDict",
            optional,
            false,
            false,
        )
...
function emit_command(io, iface, cmd_name, cmd, entries, sources, unmapped)
    ...
    typed = [
        (name = n, spec = map_type(params[n], entries, unmapped)) for
        n in sort(collect(keys(params)))
    ]
    ...
    if isempty(ret_names)
        ...
        if length(exprs) == 1
            ...
            println(
                io,
                "return (",
                join(["$(safe_ident(e.name)) = $(e.expr)" for e in exprs], ", "),
                ")",
            )
        end
    end
